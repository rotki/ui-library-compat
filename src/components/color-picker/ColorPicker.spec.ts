...
function createWrapper(options?: any) {
  return mount(ColorPicker, { ...options, stubs: { RuiIcon: true } });
}
...
describe('colorPicker', () => {
  ...
  it ('type on the input', async () => {
    ...
    expect(wrapper.emitted('input')!.at(-1)[0]).toBe('ff0000');
    ...
    expect(wrapper.emitted('input')!.at(-1)[0]).toBe('80ffff');
  });
  ...
  it('click on the UI selector', async () => {
    ...
    expect(wrapper.emitted('input')!.at(-1)[0]).toBe('804040');
  });
});
